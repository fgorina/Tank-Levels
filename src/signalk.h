...
String requestAuth(char *skserver, int skport, char *skpath)
{
  ...
  String body = "{ \"clientId\": \"" + String(device_name) + "\", \"description\": \"Tank Level Sensor\",\"permissions\": \"readwrite\"}";
...
void browseService(const char *service, const char *proto)
{
  ...
  else
  {
    ...
    for (int i = 0; i < n; ++i)
    {
      if (MDNS.hostname(i) == "signalk.local")
      {
...
void sendData(float percent, float volume)
{
  ...
  String s = update1 + me + update5 + String(percent) + update6 + String(volume) + update4;
  client.send(s);
  if (DEBUG)
  {
    Serial.print("Sent: ");
